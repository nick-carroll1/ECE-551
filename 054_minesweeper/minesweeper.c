...
board_t * makeBoard(int w, int h, int numMines) {
  ...
  for (int eachRow = 0; eachRow < h; eachRow++) {
    myBoard->board[eachRow] = malloc(w * sizeof(*myBoard->board[eachRow]));
    for (int eachCol = 0; eachCol < w; eachCol++) {
      myBoard->board[eachRow][eachCol] = UNKNOWN;
    }
  }
